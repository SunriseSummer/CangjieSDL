...
把资源关系画成树：`SdlWindow` 拥有 `Renderer`；Renderer 创建的 Texture、命令缓冲和文字会话必须在 Renderer 仍有效时使用，并且只能在该 Renderer 的拥有线程调用。命令缓冲还记录 Renderer 设备代数：窗口发生 render-target/device reset 后，旧缓冲会在绘制前拒绝 replay，应用应从 CPU 侧源数据重建，而不是继续持有失效 GPU 状态。Surface 独立存在，可先加载或生成，再由 Renderer 上传为 Texture；上传完成不代表 Surface 自动关闭，也不代表 Texture 可脱离 Renderer。Cursor 通常由应用或某个窗口工具拥有，但激活是全局可见效果，替换时要保留当前资源直到新光标生效。
...
短生命资源适合局部 try-with-resources；跨帧资源适合集中到 `Assets` 或应用状态对象，由应用退出时统一关闭。绘制事务优先用 `renderFrame` 或 try-with-resources 包住 `beginRenderPass`：body 抛错时仍会恢复 render target、scale 与 clip，且不会呈现残缺帧。集中管理减少遗漏，但要明确关闭顺序并防止业务对象任意持有资源。资源缓存可以提高性能，代价是更长生命周期和设备失效管理；动态加载后立即丢弃 Surface、保留 Texture，通常适合只绘制不读像素的图片。
...
“窗口关闭会释放所有 Surface 和 Cursor”不成立；独立资源仍由创建者负责。“Texture 是图片数据，可以在任何窗口绘制”也不成立，它绑定创建它的 Renderer 和线程。Renderer reset 也不是普通重绘信号：旧命令缓冲可能已失效，需要重建。try-with-resources 不只为异常服务，它还把正常路径的关闭顺序写进结构。最后，Surface 转 Texture 不是移动所有权：转换后两个资源同时存在，直到各自关闭。
